...
- How context packets communicate with subagents
...
## Phase 1-4: Same as Beginner Pattern

Phases 1-4 (Session Start, Brainstorm, Branch, Backlog Development) work identically to the Beginner Pattern. See [beginner-pattern.md](./beginner-pattern.md) for the detailed file traces.
...
sequenceDiagram
    ...
    loop For each task
        ...
        Hook->>Hook: validate-context-packet.sh
        Orchestrator->>Task: Dispatch code-implementer
        Task->>Impl: Execute with context packet
...
│   Lines 37-53: Context packet preparation guidance                  │
...
│   Lines 136-190: Context Packet Format                              │
│   Lines 192-211: Context Packet Checklist                           │
...
sequenceDiagram
    ...
    participant Hook as hooks.json
    participant Validate as validate-context-packet.sh
    participant Task as Task Tool
    participant Impl as code-implementer

    Orchestrator->>Orchestrator: Prepare context packet
    Orchestrator->>Task: Dispatch code-implementer

    Task->>Hook: PreToolUse (Task)
    Hook->>Validate: Execute validate-context-packet.sh
    ...
    Task->>Impl: Agent receives context packet
...
```
Orchestrator prepares context packet and dispatches
     │
...
│     "matcher": "Task",                                              │
│     "hooks": [                                                      │
│       { "command": "validate-context-packet.sh" }                   │
│     ]                                                               │
│   }                                                                 │
└─────────────────────────────────────────────────────────────────────┘
     │
     ▼
┌─────────────────────────────────────────────────────────────────────┐
│ hooks/validate-context-packet.sh (lines 1-99)                       │
│                                                                     │
...
The orchestrator prepares a context packet like this (from `orchestrating-subagents/SKILL.md` lines 140-190):
...
- [x] `hooks/validate-context-packet.sh` **NEW**
...
2. **Context packets are the communication protocol**: Complete context packets (with Purpose, Failure Modes, Skills) ensure subagent success.
...
5. **The validate-context-packet hook helps**: It warns if your context packet is missing sections.
